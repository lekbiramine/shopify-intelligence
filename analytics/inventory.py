...
def _get_stock_products(store_id: int, min_qty: int, max_qty: int) -> list[dict]:
    """
    ...
    with get_cursor() as cursor:
        cursor.execute(sql, {"store_id": store_id, "min_qty": min_qty, "max_qty": max_qty})
        results = cursor.fetchall()

    return [dict(r) for r in results]


def get_low_stock_products(store_id: int) -> list[dict]:
...
def get_critical_stock_products(store_id: int) -> list[dict]:
    results = _get_stock_products(store_id, 1, constants.CRITICAL_STOCK_THRESHOLD)
    logger.info(f"Found {len(results)} critical stock variants.")
    return results


def get_out_of_stock_products(store_id: int) -> list[dict]:
    results = _get_stock_products(store_id, 0, 0)
